...
The last two rows learn no dictionary, and only the calibrated one is correctly
oriented on all ten families (\S\ref{sec:detector}).}
\label{tab:boundary}
{\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
...
% src: comb_{fmc,sonics}_HNULL/analytic_null_auc.csv | gates ledger R27.42, R27.45
It reaches $0.9445$ $[0.9416, 0.9474]$ on FakeMusicCaps and, on SONICS, $0.8286$
macro with $0.7925$ $[0.7887, 0.7963]$ pooled. Udio reads $0.586$ and $0.579$, and no
margin is tied at $0$, $\mathcal{N}$ and $\mathcal{P}$ being disjoint by construction.
It is the only score we tested correctly oriented on all ten generator families. Its gate profile is the comb arm's own, and the
