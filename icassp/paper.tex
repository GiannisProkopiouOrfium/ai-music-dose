...
\renewenvironment{thebibliography}[1]{\section{References}\list
 {[\arabic{enumi}]}{\settowidth\labelwidth{[#1]}\leftmargin\labelwidth
  ...
  \itemsep 0pt \parsep 0pt \topsep 2pt
  \usecounter{enumi}}%
...
\address{%
  ...
  $^5$Archimedes/Athena R.C., Athens, Greece}
...
held out, gives $0.744$ and $0.382$. The gap closes at about $50$ generated tracks
in $6{,}400$ on SONICS, as a threshold rather than a ramp. We then propose one with no
...
null on its own residual that excludes every harmonic of every candidate: the only
score we tested correctly oriented on all ten generator families, and above the
best training-free baseline on both benchmarks ($+0.027$ and $+0.074$) with no
random draw.
\end{abstract}
...
a working detector and an inverted one.

Our object of study is the blurred-atom score of Afchar and Hennequin
...
real)''. Their title and conclusion nonetheless frame the method as zero-shot,
and it is that reading a deployer inherits. What neither supplies is a
number. The dictionary is learned from a matrix that \emph{contains the corpus
being scored}, which is by construction majority generated audio, so the
estimator is transductive and the open question is its \emph{dose}. It is small,
it is sharp, and on SONICS it separates an inverted score from a working one.

We do not repair that estimator; we propose one that never incurs a dose. A
...
benchmarks at full corpus with held-out dictionaries, and on SONICS crossed as a
threshold rather than a ramp (\S\ref{sec:boundary}); (ii) the sign result: below the dose
...
literature (\S\ref{sec:deployment}); and (v) \textbf{a detector with no dose at all}, a
...
(\S\ref{sec:detector}). Code, the gate battery, the provenance of every number here
and a record of every withdrawn claim are released at
...
\cite{afchar2025fourier} turn this into the fakeprint and a detection criterion;
...
we reproduce it, and it remains the strongest label-free result on either
benchmark. We add a measurement of what its \emph{protocol} contributes, which
...
\cite{comanducci2025fakemusiccaps} and SONICS \cite{rahman2025sonics}.

Two criteria we use are not ours: Zhou and Wang \cite{zhou2026fragile} require a
...
\emph{density}. We rejected six density formulations
...
two-corpus scope should be judged. The nearest analogues to
Fig.~\ref{fig:contamination}(a,b) are contamination robustness in anomaly
detection \cite{pang2019deviation}, which asks the \emph{inverse} question, and
...
\textbf{signed macro AUC} is the unweighted mean over the five generator families
...
on SONICS), loudness-normalised to $-14$\,LUFS (loudness
units relative to full scale) and capped
at a common duration ($9$\,s on FakeMusicCaps, $120$\,s on SONICS), matched over
the full corpus rather than a split; \S\ref{sec:confounds} shows why that is not
...
$r_i$ is inflated; if the fit reals are also the scored reals, real music scores
higher and the AUC inverts for reasons unrelated to generated audio. On
...
\textbf{All deployer-side numbers here are held out}: any score read off a fitted
...
full corpus, $K{=}20$, full resolution; deployer rows are held out over three draws.
...
the scored corpus (published) & \textbf{0.9917} & \textbf{0.9389} \\
% src: nmf_fmc_fitsonics_FULL/, nmf_sonics_fitfmc_FULL/nmf_peak_auc_lvl.csv
a different corpus (inductive) & 0.5733 & 0.7123 \\
% src: nmf_{fmc,sonics}_curve_FINAL/nmf_peak_auc_lvl.csv, 0% cell, 3 seeds
real music only, held out & $0.744{\pm}0.020$ & $0.382{\pm}0.043$ \\
\midrule
...
signal, decoder combs being architecture-determined \cite{pons2021upsampling}. Second, \textbf{the sign of the collapse is corpus-dependent.}
FakeMusicCaps degrades to a usable $0.744$, what a deployer actually gets,
whereas SONICS \emph{inverts}, real music scoring higher than generated; we say
``inverts'' only about SONICS. One ordering is unexplained: on FakeMusicCaps the
...
% released, not quoted: those cells predate \S\ref{ssec:holdout}'s correction.
(\S\ref{sec:conc}). The collapse is not specific to NMF: $\ell_1$ sparse coding
and $k$-means atoms reproduce it on both corpora.

\subsection{The dose}
\label{ssec:dose}
Fig.~\ref{fig:contamination}(a,b) fits the dictionary on half the real catalogue
plus a known number of unlabelled generated tracks, and scores the other half of
the reals together with all unused generated audio.

On SONICS the mean curve appears to rise smoothly between $23$ and $70$
...
$0.47$ against $0.009$--$0.012$, with no draw between: the mean's smoothness is an
artefact of averaging draws on opposite sides of a transition.
...
large sample collection'' of \cite{afchar2026finding}: $0.7\%$ of the unlabelled
set, and a threshold rather than a gradual requirement. The reading is that the
factorisation can or cannot afford one of its $K$ atoms for the
% src: nmf_sonics_K{5,10,40}_knee/nmf_peak_auc_lvl.csv, 3 seeds, bins4779
comb, and the atom budget tests it: the flipping dose falls monotonically as $K$
grows, from $168$--$341$ tracks at $K{=}5$ to ${\approx}47$ at $K{=}20$, with
$K{=}40$ already flipped at the smallest dose sampled.

Below ${\approx}5\%$ contamination on FakeMusicCaps one generator swings $0.21$
...
oriented on all five families; chain matching takes it to $0.60$, and costs the
comb arm on that corpus $0.217$ ($0.84 \!\to\! 0.63$). Every value below is
measured after that chain.
...
arm's own SONICS macro from $0.6203$ to $0.8468$, because the Udio families are
\emph{inverted} while the chirp families are not. $|$AUC$|$ measures class
information, not detector performance.
...
\cite{zhou2026fragile}'s criterion applied one level up. Pooled, the dictionary
score's correlation with the comb flips to $-0.08$ while energy stays $+0.20$: a
failed between-class ordering, not lost sensitivity within a family
($\rho = 0.41$--$0.74$ inside chirp).
...
$|x - \mathrm{med}| / \mathrm{IQR}$, with IQR the interquartile range. Over four arm-by-corpus cells it costs
$0.005$/$0.018$ where no family inverts and gains $0.068$/$0.022$ where one does,
DeLong \cite{delong1988comparing} $p = 3.1 \times 10^{-57}$.

\noindent\textbf{Confound gates.} Every reported number carries an eleven-gate
battery, released in full with the code: channel, level, duration, silence, label
...
Nine pass. Channel and level miss a criterion fixed in advance at $0.60$,
separating the classes at $0.61$--$0.63$ on both corpora: marginal failures,
precisely estimated at $n = 32{,}960$ and so not noise, but bounded:
bandwidth-matched twins move the scores by only $-0.010$ and $-0.003$. We bound the
residual channel rather than remove it. Decoder-spacing concentration is
\emph{not applicable} to the NMF arm, a skip and not a pass.
...
source on $94.7$--$98.0\%$ of pairs (Wilcoxon $p < 2.1 \times 10^{-45}$) while
phase-only Griffin--Lim sits at chance, so the comb arm responds to deconvolution
...
the analysis resolution, and both benchmarks are \emph{distributed} at
$16$\,kHz, so nothing survives above $8$\,kHz whatever container rate the chain of
\S\ref{sec:method} uses; \cite{afchar2026finding} cites that cutoff as unsuited
and a reason to work at $44.1$\,kHz instead.
...
$0.9386$, and at their physical resolution (${\approx}2.7$\,Hz per bin) $0.9878$
and $0.9293$, at most $0.010$ either way; coarser binds, $0.9457$/$0.8368$ at $445$
bins. The requirement is a resolution, not a dimension, re-derived when the sample
rate changes.
...
At $M{=}2$ the prior-restricted score of \S\ref{sec:detector} attributes by a
different route: the candidate it fires on concentrates per generator in
...
Suno's decoder is proprietary and did not inform the candidate set, so that is
attribution on a held-out architecture.
...
(\S\ref{sec:confounds}), which is why we quote the two-sided form. Having no
...
Restriction alone helps, since the maximum of a noisy autocorrelation over $K$ lags
grows as $\sigma\sqrt{2\ln K}$; but the restricted score still inherits each track's
...
smoothness cancels. \emph{Which} null is the detector. $\mathcal{N}$ is every lag in $[40, 600]$\,Hz
except $\pm 1$ bin around $k\Delta$, for each candidate spacing $\Delta$ and
\emph{every} integer $k$: $441$ lags on FakeMusicCaps, $253$ on SONICS. Deleting
...
disjoint by construction. \textbf{It is the only score here correctly oriented on all
...
turns an inversion into a miss, a lesser and different thing: three statistics agree
Udio leaves no comb inside these benchmarks' band.
...
dictionary score on SONICS recalls $67.4\%$ of generated tracks, which at a $1\%$
base rate is a precision of $0.127$: nearly seven false accusations per correct
...
$2.4\times$ as much, recomputing an autocorrelation per decoy set. It degrades $+0.2$/$+0.9$
...
AUC, bought by ${\approx}0.7\%$ unlabelled generated audio and, on SONICS, crossed as
a threshold rather than a ramp. Three things should accompany any label-free number: which unlabelled
audio the estimator saw, whether fitted rows were scored, and the signed
per-generator AUC.

\noindent\textbf{Limitations.} Two $16$\,kHz benchmarks, channel labelled by
construction, two gates failing marginally on both; M6 \cite{li2026m6} or Echoes
\cite{pascu2026echoes} would test the dose under stronger shortcut controls. The
corpora differ in how their halves were assembled, and channel matching equalises
rate, level and duration but not encoding history, a candidate explanation for the
corpus-dependent sign. Draws are three across the range and ten at the
% src: analysis_h4_fmc/, analytic_null_auc.csv recall_at_q95 (ledger R27.93)
transition; $\sigma$, $F$ and the prior's depth were selected once on FakeMusicCaps;
and the
calibration of \S\ref{sec:detector} was devised after the one-sided score fell
short on SONICS, so its gain \emph{there} is post-hoc, the FakeMusicCaps gain
resting on a corpus that never motivated it. Its recall at a fixed quantile is
...
% src: data/processed/robustness_hmargin/robustness_auc_eer.csv (ledger R27.58, R27.95)
The prior is a scope limit and not only a design choice: it assumes a strided
transposed-convolution decoder, and a synthesiser built without one leaves no comb
to find. One manipulation defeats it outright: pitch-shift resamples, moving $\Delta$ off
all six candidate frequencies, and drives the score \emph{below} chance ($0.4665$),
a signed inversion rather than a degradation. A log-frequency axis
...
generator match, and the hybrid case \cite{afchar2026finding} names as open.%
\label{endofbody}
